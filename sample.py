...
class PerContactDictionaryValue(object):
    ...
    def delete_list(self, names) -> list:
        deleted = []
        for name in names:
    ...
    def set_working_list(self, dictionary):
        self.working_list = dictionary
        try:
            self.alternative_rand_li = self.dict[dictionary][:]
        except KeyError:
